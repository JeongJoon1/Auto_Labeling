...
from scrapy.pipelines.images import ImagesPipeline, NoimagesDrop, ImageException
import hashlib
import functools
import hashlib
import six
...
from imagecrawler.spiders.pexels import PexelsSpider
from imagecrawler.spiders.pixabay import PixabaySpider
...
class ImagecrawlerPipeline(ImagesPipeline):
    ...
    pexels_name = PexelsSpider.folder_name
    pixabay_name = PixabaySpider.folder_name
    ...
    def file_path(self, request, response=None, info=None, *, item=None):
        ...
        if request.url.find("pexels") != -1:
            file_name = url.split('/')[4]
            print(file_name)
            save_folder_name = self.pexels_name+ '/'
            return save_folder_name + file_name + '.jpeg'

        elif request.url.find("pixabay") != -1:
            file_name = url.split('/')[4]
            print(file_name)
            save_folder_name = self.pixabay_name + '/'
            return save_folder_name + file_name + '.jpg'
